...
Процедура ЗаписатьПродажу(Команда)
	НовыйДокумент = ЗаписатьПродажуНаСервере(); // ДокументСсылка.ПродажаБилетов
	ОповеститьОбИзменении(НовыйДокумент);
	ПоказатьОповещениеПользователя(НовыйДокумент, 
...
Процедура РассчитатьСуммуДокумента()
	...
	ЗаполнитьДекарациюБаллыКлиента();
//	СуммаИтого = ПозицииПродажи.Итог("Сумма") - БаллыКСписанию;
КонецПроцедуры	

&НаСервере
Процедура СброситьЗначенияФормы()
...
// Записать продажу на сервере.
// 
// Возвращаемое значение:
//  ДокументСсылка.ПродажаБилетов - Записать продажу на сервере
&НаСервере
...
&НаСервере
Процедура ЗаполнитьДекарациюБаллыКлиента()
	...
	|	БонусныеБаллыКлиентоОстатки.СуммаОстаток
	|ИЗ
	|	РегистрНакопления.БонусныеБаллыКлиентов.Остатки(, Клиент = &Клиент) КАК БонусныеБаллыКлиентоОстатки";
...
Процедура СброситьИзбранныеТовары()
	Для Каждого Стр Из ДинамическиеЭлементы Цикл
		Стр.Активность = Ложь;
